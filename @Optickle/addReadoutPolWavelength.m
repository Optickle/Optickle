...
function opt = addPolWavelength(opt, name, linkIn, portIn)
  ...
  for kk=1:size(llamb,1)
    ...
    for jj=1:size(llamb,1)
      if llamb(kk) == llamb(jj)
        mDich = [mDich;1 llamb(kk) 1;1 llamb(kk) 0];
      else
        mDich = [mDich;0 llamb(kk) 1;0 llamb(kk) 0];
      end
    end  
    nameSplit = sprintf('%s%s%d', name, 'SplitP', llamb(kk)/1e-9);
    opt = addMirror(opt, nameSplit, 45, 0, mDich);
    nameSplit = sprintf('%s%s%d', name, 'SplitS', llamb(kk)/1e-9);
    opt = addMirror(opt, nameSplit, 45, 0, mDich);
  end
  
  nameSplit = sprintf('%s%s%d', name, 'SplitP', llamb(1)/1e-9);
  opt = addLink(opt, nameSplitPol, 'fr', nameSplit, 'fr', 0);
  nameSplit = sprintf('%s%s%d', name, 'SplitS', llamb(1)/1e-9);
  opt = addLink(opt, nameSplitPol, 'bk', nameSplit, 'fr', 0);
  for kk=2:size(llamb,1)
    for ps=['P','S']
      nameSplitIn = sprintf('%s%s%s%d', name, 'Split', ps, llamb(kk-1)/1e-9);
      nameSplitOut = sprintf('%s%s%s%d', name, 'Split', ps, llamb(kk)/1e-9);
  ...
  for ps=['P','S']
    nameSink = sprintf('%s%s%d', name, ps, llamb(end)/1e-9);
    nameSplitIn = sprintf('%s%s%s%d', name, 'Split', ps, llamb(kk)/1e-9);
    opt = addSink(opt, nameSink);
    opt = addLink(opt, nameSplitIn, 'bk', nameSink, 'in', 0);
  end
