...
function opt = Optickle(varargin)
  ...
  switch( nargin )
    ...
    case 1
      ...
      % vFrf, lambda = 1064e-9
      [opt.vFrf, opt.lambda, opt.pol] = makeRFvector(arg, 1064e-9, 1);
      else
        error([errstr 'an argument of type %s.'], class(arg));
      end
    case 2
      % vFrf, lambda
      [opt.vFrf, opt.lambda, opt.pol] = makeRFvector(varargin{:}, 1);
    case 3
      % vFrf, lambda, pol
      [opt.vFrf, opt.lambda, opt.pol] = makeRFvector(varargin{:});
